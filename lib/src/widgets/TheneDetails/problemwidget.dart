...
final PanelController _panelSendToOrderPanelCo0ntroller = PanelController();
int _counter = 31;
int _cc = 0;
int score = 0;
int _manyOption = 3;
bool _isOpenOrderPanel = true;
...
class _ProblemWidgetState extends State<ProblemWidget> {
  ...
  Widget _slidingUpOptionResult(Size media, List<UnitQuestion> questions) {
    return SlidingUpPanel(
        slideDirection: SlideDirection.UP,
        onPanelOpened: () => setState(() {
              _isOpenOrderPanel = false;
            }),
        onPanelClosed: () => setState(() {
              _isOpenOrderPanel = true;
            }),
        controller: _panelSendToOrderPanelCo0ntroller,
          ...
          child: Row(
            ...
            children: [
              ...
              (_isOpenOrderPanel)
                  ? TextButton.icon(
                      onPressed: () {
                        _panelSendToOrderPanelCo0ntroller.open();
                        setState(() {
                          _isOpenOrderPanel = false;
                        });
                      },
                  ...
                  : TextButton.icon(
                      onPressed: () {
                        _panelSendToOrderPanelCo0ntroller.close();
                        setState(() {
                          _isOpenOrderPanel = true;
                        });
                      },
  ...
  Widget straightEquations(int index, List<UnitQuestion> questions) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      SizedBox(
          height: screenMediaSales.width * 0.1,
          width: screenMediaSales.width * 0.1,
          child: OutlinedButton(
              onPressed: () {
                      ...
                      lineTouchData: LineTouchData(
                        ...
                        touchCallback: (event, touchResponse) {
                          if (event.isInterestedForInteractions &&
                            ...
                            log(" Touch en X ${convertedX}, Touch en Y ${convertedY}");
                            final List<FlSpot> data = [
                              FlSpot(convertedX, convertedY),
                              FlSpot(0, convertedY),
                              FlSpot(convertedX, convertedY),
                              FlSpot(convertedX, 0),
                            ];
                            if (_textanswerSended1 == "point1") {
                              setState(() {
                                _data2P = data;
                              });
                            } else {
